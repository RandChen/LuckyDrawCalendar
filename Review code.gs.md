...
這是我們為了**提升回覆速度**與**資料即時性**而加入的資深級功能：
...
*   **預留強制刷新後門 (Line 115)**：為了使用者手動改表單的極端情境，API 現在支援接收 `?nocache=true` 參數。一旦偵測到這個參數，API 就會強制刪除記憶體快取，重新去讀一次最新的試算表。
*   **效能效益**：這讓一般使用下的 API 反應時間從原本的 1000ms+ 縮短到幾乎瞬間完成。
